...
public class FTPServerService extends Service implements Runnable {
    ...
    public void onCreate() {
        ...
        intentFilter.addAction(Intent.ACTION_MEDIA_UNMOUNTED);//sd卡不可用
    ...
    void setupListener() throws IOException {
        listenSocket = new ServerSocket(); //打开一个socket
        ...
        while (!shouldExit) {
            if (acceptWifi) {
                if (wifiListener != null) {
                    if (!wifiListener.isAlive()) { //是否活着
                        myLog.l(Log.DEBUG, "Joining crashed wifiListener thread");
                        try {
                            wifiListener.join(); //等待该线程终止。 让他运行完了， 再运行别的线程！
                        } catch (InterruptedException e) {
                        }
                        wifiListener = null;
                    }
